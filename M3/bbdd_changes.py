...
def insert_data_weapons(game_id):
    ...
    for weapon_item in diccionarios.player_dict['weapons_inventory']:
        ...
        if weapon_name == "Wood Sword":
            weapon_type = "uses_left_woodsword"
        elif weapon_name == "Sword":
            weapon_type = "uses_left_sword"
        elif weapon_name == "Wood Shield":
            weapon_type = "uses_left_woodshield"
        elif weapon_name == "Shield":
            weapon_type = "uses_left_shield"
            
        if row_count > 0:
            update_query = """
                    UPDATE game_weapons
                    SET quantity = %s, uses = %s, %s = %s
                    WHERE weapon_name = %s AND game_id = %s
                """
            cursor.execute(update_query, (quantity, uses, weapon_type, lives_remaining, weapon_name, game_id))
        else:
            insert_query = """
                    INSERT INTO game_weapons (game_id, weapon_name, %s, quantity, uses)
                    VALUES (%s, %s, %s, %s, %s)
                """
            cursor.execute(insert_query, (weapon_type, game_id, weapon_name, lives_remaining, quantity, uses))
...
def download_data_mysql(game_id):
    ...
    weapons_equipped_query = "SELECT weapon_name FROM gamºe_weapons WHERE equipped = 1 AND game_id = %s"
