...
def PantallaPrincipal():
    ...
    def queries(consulta):
        if consulta==1:
            ...
            rows.append(cursor.fetchone())#lo agregamos a la variable que habiamos creado antes, como solo es una fila usamos fetchone()
            for i in range(len(rows)):#imprimos por pantalla
                if i == 0:
                    print("| {} | {} | {} | {} |".format(rows[i][1].ljust(10),str(rows[i][0]).ljust(19),str(rows[i][2]).rjust(22),str(rows[-1][0]).rjust(30)))
                ...
                if opc !="Back":
                    ...
                    while opt != "Continue":
                        ...
                        if opt != "Continue":
                            ...
                            historialPrompt(prompt, opt)
                            cursor.execute("select game_id,region from game where game_id = (select max(game_id) from game)")
                            partida, region = cursor.fetchone()
                            return partida,region
